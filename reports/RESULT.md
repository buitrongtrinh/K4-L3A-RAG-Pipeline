...
Both configurations use the same golden dataset, prompt, generator and evaluator. PageIndex fallback is excluded because it is an optional provider-dependent path.
...
The runner executed all 30 combinations (15 cases × 2 configurations) after building a 475-chunk BGE-M3/Chroma collection. The per-case answers, metric values, rationale, source IDs and retrieval time are stored in `group_project/evaluation/ab_results.json`.
